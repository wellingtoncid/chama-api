...
try {
    ...
    // Requisitos de arquivos (Verifique se esses caminhos existem exatamente assim)
    ...
    // Proteção: não valida se o token for lixo ou string "undefined"
    $loggedUser = (empty($token) || $token === 'undefined' || $token === 'null') ? null : $auth->validateToken($token);

    /**
     * 5. ROTEAMENTO UNIVERSAL (Padrão original preservado)
     */
    switch ($endpoint) {
        ...
        case 'my-matches':
        case 'finish-freight':
            $ctrl = new FreightController($db);
            echo json_encode($ctrl->handle($method, $endpoint, $data, $loggedUser));
            break;
    ...
    // Throwable captura erros fatais que o Exception comum não pega
