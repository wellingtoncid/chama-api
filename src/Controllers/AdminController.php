...
class AdminController {
    ...
    private function authorize($loggedUser = null, $minRole = 'MANAGER') {
        ...
        if ($minRole === 'users.manage' || $minRole === 'users.view') {
            if (\App\Core\Auth::hasPermission($minRole, $user['id'] ?? null)) return;
        ...
        if ($requiredRole === 'admin') {
            ...
            if (!$isUserManager) throw new Exception("Acesso negado: Requer nível GERENTE ou superior.", 403);
        } else {
            if (!$isUserManager && $userRole !== $requiredRole) throw new Exception("Acesso negado: Permissão insuficiente.", 403);
        }
    }
    ...
    public function listLogs($data, $loggedUser) {
        if (!$this->adminRepo) $this->adminRepo = $this->repo;
    ...
    public function manageAds($data, $loggedUser) {
        ...
        if ($action === 'pause' && $id) {
            ...
            if ($success) $this->repo->saveLog($loggedUser['id'], $loggedUser['name'], 'PAUSE_AD', "Pausou anúncio #{$id}", $id, 'AD');
            return Response::json(["success" => $success]);
        }
        if ($action === 'activate' && $id) {
            $success = $this->adRepo->activateAd($id);
            if ($success) $this->repo->saveLog($loggedUser['id'], $loggedUser['name'], 'ACTIVATE_AD', "Ativou anúncio #{$id}", $id, 'AD');
            return Response::json(["success" => $success]);
        }
        if ($action === 'renew' && $id) {
            $days = isset($data['days']) ? (int)$data['days'] : 30;
            $success = $this->adRepo->renewAd($id, $days);
            if ($success) $this->repo->saveLog($loggedUser['id'], $loggedUser['name'], 'RENEW_AD', "Renovou anúncio #{$id} por {$days} dias", $id, 'AD');
            return Response::json(["success" => $success]);
        }
    ...
    public function getSettings($data, $loggedUser) {
        ...
        if (!$loggedUser && $this->loggedUser) $loggedUser = $this->loggedUser;
        try { $this->authorize($loggedUser, 'ADMIN'); }
        catch (\Throwable $e) { return Response::json(["success" => false, "message" => "Não autorizado: " . $e->getMessage()], 403); }
        try {
            ...
            } catch (\Throwable $e) { error_log("Tabela plans não existe ou erro: " . $e->getMessage()); }
        ...
        try {
            ...
            if (!$key && isset($data['site_name'])) {
                ...
                foreach ($allowedKeys as $settingKey) {
                    if (isset($data[$settingKey])) $this->upsertSetting($settingKey, $data[$settingKey]);
                }
            ...
            if (!$key) return Response::json(["success" => false, "message" => "Chave não informada"], 400);
    ...
    public function findMatchingDrivers($data, $loggedUser) {
        ...
        try {
            ...
            if (!$freightId) return Response::json(["success" => false, "message" => "ID do frete não informado"], 400);
            ...
            if (!$freight) return Response::json(["success" => false, "message" => "Frete não encontrado"], 404);
            if (!$freight['origin_lat'] || !$freight['origin_lng']) return Response::json(["success" => false, "message" => "Frete não possui coordenadas de origem. Use a página de edição para geolocalizar."], 400);
            ...
            foreach ($drivers as &$driver) {
                if (isset($driver['available_equipment'])) $driver['available_equipment'] = json_decode($driver['available_equipment'], true) ?? [];
                if (isset($driver['certifications'])) $driver['certifications'] = json_decode($driver['certifications'], true) ?? [];
            }
            return Response::json([
                "success" => true,
    ...
    public function updateDriverLocation($data, $loggedUser) {
        ...
        try {
            ...
            if (!$userId) return Response::json(["success" => false, "message" => "ID do usuário não informado"], 400);
            if (!$homeLat || !$homeLng) return Response::json(["success" => false, "message" => "Coordenadas inválidas"], 400);
    ...
    private function recalculateProfileCompleteness(int $userId): void {
        ...
        } catch (\Throwable $e) { error_log("ERRO recalculateProfileCompleteness: " . $e->getMessage()); }
    }
    ...
    public function getActivityLogs($data, $loggedUser) {
        ...
        } catch (\Throwable $e) { return Response::json(["success" => false, "message" => "Erro ao carregar atividades"]); }
    }
    ...
    public function manualAddCredits($data, $loggedUser) {
        ...
        try {
            ...
            if (!$stmtCheck->fetch()) throw new Exception("Usuário não encontrado");
