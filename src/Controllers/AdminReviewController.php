...
class AdminReviewController {
    ...
    public function approveReview($data, $loggedUser = null) {
        ...
        if (!$reviewId) return Response::json(["success" => false, "message" => "ID da avaliação é obrigatório."], 400);
        try {
            $reviewRepo = new \App\Repositories\ReviewRepository($this->db);
            $review = $reviewRepo->findById($reviewId);
            if (!$review) return Response::json(["success" => false, "message" => "Avaliação não encontrada."], 404);
            if ($review['status'] !== 'pending') return Response::json(["success" => false, "message" => "Apenas avaliações pendentes podem ser aprovadas."], 400);
