...
class AdminUserController {
    ...
    public function createUser($loggedUser) {
        try {
            ...
            if (!$input) return Response::json(["success" => false, "message" => "Dados inválidos"], 400);

            $userType = strtoupper($input['user_type'] ?? '');
            if ($userType === 'DRIVER') {
                $requiredFields = ['name', 'document', 'email', 'password'];
                foreach ($requiredFields as $field) {
                    if (empty($input[$field])) return Response::json(["success" => false, "message" => "O campo $field é obrigatório para Motorista"], 400);
                }
                $cpf = preg_replace('/\D/', '', $input['document']);
                if (strlen($cpf) !== 11) return Response::json(["success" => false, "message" => "CPF deve ter 11 dígitos"], 400);
            } elseif ($userType === 'COMPANY') {
                $requiredFields = ['name', 'document', 'email', 'password', 'owner_name'];
                foreach ($requiredFields as $field) {
                    if (empty($input[$field])) return Response::json(["success" => false, "message" => "O campo $field é obrigatório para Empresa"], 400);
                }
                $cnpj = preg_replace('/\D/', '', $input['document']);
                if (strlen($cnpj) !== 14) return Response::json(["success" => false, "message" => "CNPJ deve ter 14 dígitos"], 400);
            } elseif ($userType === 'SYSTEM') {
                $requiredFields = ['name', 'email', 'password', 'role'];
                foreach ($requiredFields as $field) {
                    if (empty($input[$field])) return Response::json(["success" => false, "message" => "O campo $field é obrigatório para usuário do Sistema"], 400);
                }
            } else {
                return Response::json(["success" => false, "message" => "Tipo de usuário inválido. Use: DRIVER, COMPANY ou SYSTEM"], 400);
            }

            if (!filter_var($input['email'], FILTER_VALIDATE_EMAIL)) return Response::json(["success" => false, "message" => "Email em formato inválido"], 400);
            ...
            if ($userType === 'SYSTEM') {
                ...
                if ($stmtCheck->fetch()) return Response::json(["success" => false, "message" => "Este email já está cadastrado"], 400);
    ...
    public function verifyUser($data, $loggedUser) {
        try {
            ...
            if ($this->repo->setUserVerification($id, $status)) {
                ...
                if ($status == 1) $this->notif->notify($id, "Perfil Verificado!", "Sua conta foi aprovada manualmente pelo administrador.");
                return Response::json(["success" => true]);
            }
    ...
    public function getUserDetails($data, $loggedUser) {
        $this->authorize($loggedUser);
        $id = $data['id'] ?? null;
        if (!$id) return Response::json(["success" => false, "message" => "ID não fornecido"], 400);
    ...
    public function updateUser($data, $loggedUser) {
        $this->authorize($loggedUser);
        if (!isset($data['id'])) return Response::json(["success" => false, "message" => "ID do usuário é obrigatório"], 400);
    ...
    public function getUserNotes($data, $loggedUser) {
        ...
        if (!$userId) return Response::json(["success" => false, "message" => "user_id é obrigatório"]);
