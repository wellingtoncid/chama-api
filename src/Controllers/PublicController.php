...
class PublicController
{
    ...
    public function getPublicPosts($data)
    {
        ...
        try {
            ...
            // Fretes: exclusivo de COMPANY
            if (in_array($user['user_type'], ['COMPANY', 'SHIPPER'])) {
                $freights = $this->freightRepo->getPublicPostsByUser($userId);
                foreach ($freights as &$f) {
                    $f['type'] = 'freight';
                }
                $results = array_merge($results, $freights);
            }

            // Anúncios: exclusivo de ADVERTISER
            if ($user['user_type'] === 'ADVERTISER') {
                $ads = $this->adRepo->getAdsByUserId($userId);
                foreach ($ads as &$a) {
                    $a['type'] = 'ad';
                }
